...
[loss3, acc3] = interface(X, y, XT, yT, Class, n_layers, stuc_layers, algorithm ...
    , lambda, batch_size, n_iterations, n_save_interval, step_size, scr_params);
time = toc;
fprintf('Time: %f seconds \n', time);
loss3 = [X_axis, loss3];
acc3 = [X_axis, acc3];
...
%     ma = min([aa1, aa2]);
    ...
    hold on,semilogy(acc3(1:ba:end,1), abs(acc3(1:ba:end,2) - ma),'k--+','linewidth',1.2,'markersize',4.5);
    hold off;
    xlabel('Number of Steps');
    ylabel('Accuracy');
    axis([0 n_iterations/n_save_interval 0.8 aa]);
    legend('SGD', 'Adam', 'SCR');
end
